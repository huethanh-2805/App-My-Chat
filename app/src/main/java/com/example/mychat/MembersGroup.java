                            ...
                            if (memberReferences != null && !memberReferences.isEmpty()) {
                                ...
                                Tasks.whenAllComplete(memberTasks)
                                        .addOnCompleteListener(task1 -> {
                                            ...
                                            CustomListMembers adapter = new CustomListMembers(MembersGroup.this, R.layout.activity_custom_list_members, memberUsernamesArray, memberRolesArray, memberAvatarsArray);
                                            listView.setAdapter(adapter);
                                        });
                            } else {
                                Toast.makeText(MembersGroup.this, "No members in the group", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(MembersGroup.this, "Member not found in group document", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(MembersGroup.this, "Group document not found", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(MembersGroup.this, "Error fetching group document: " + task.getException(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
